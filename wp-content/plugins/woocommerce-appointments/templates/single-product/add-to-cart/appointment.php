	...
	if ( $product->get_qty() > 1 && $product->get_qty_max() > 1 && ! $product->is_sold_individually() ) {
		...
		woocommerce_quantity_input(
			[
				...
				'input_value' => ( isset( $_POST['quantity'] ) ? wc_stock_amount( wc_clean( wp_unslash( $_POST['quantity'] ) ) ) : 1 ), // phpcs:ignore WordPress.Security.NonceVerification, WordPress.Security.ValidatedSanitizedInput.InputNotSanitized
			]
		);
